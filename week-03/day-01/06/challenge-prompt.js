...
function toyArrToObj(arrayOfToys){
  ...
  arrayOfToys.forEach(function(toy){
    var newKeyVal = {name: toy};
    toyArrayOfObjs.push(newKeyVal)
  })
...
function createCustomObject(objectArr){
  ...
  for (var i = 0; i < toyInventoryArrayOfObjects.length; i++) {
    var toyLineItem = toyInventoryArrayOfObjects[i].toyLine;
    var titleItem = toyInventoryArrayOfObjects[i].title;

    //Create toyline objects if they don't exist yet
    if (!customToyLineObj[toyLineItem]) {
      customToyLineObj[toyLineItem] = {};
      customToyLineObj[toyLineItem]["toyLine"] = toyLineItem;
      customToyLineObj[toyLineItem]["toyLineToys"] = [];
      customToyLineObj[toyLineItem]["totalToysInToyLine"] = 0;
    }
    //Push each toy title to object
    customToyLineObj[toyLineItem].toyLineToys.push(titleItem) ;
    customToyLineObj[toyLineItem].totalToysInToyLine += toyInventoryArrayOfObjects[i].stock;
